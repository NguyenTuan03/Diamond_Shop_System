...
import com.diamond_shop.diamond_shop.entity.DiamondPriceNatureEntity;

import java.util.List;

@Repository
public interface DiamondPriceNatureRepository extends JpaRepository<DiamondPriceNatureEntity,Integer>{
    @Query("SELECT d FROM DiamondPriceNatureEntity d WHERE d.shape = :shape")
    List<DiamondPriceNatureEntity> findAllByShape(@Param("shape") String shape);

    @Query("SELECT COUNT(d) > 0 FROM DiamondPriceNatureEntity d WHERE d.shape = :shape")
    Boolean existsByShape(@Param("shape") String shape);

    @Modifying
    @Transactional
    @Query("UPDATE DiamondPriceNatureEntity s " +
